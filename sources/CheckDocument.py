...
class CheckDocument():
    def __init__(self, path_to_JSON) -> None:
        ...
        self.document = Document(self.settings["Path-to-file"])
        self.strf = docx2txt.process(self.settings["Path-to-file"]).splitlines()
    ...
    def startAnalize(self):
        #self.strf = docx2txt.process(self.settings["Path-to-file"]).splitlines()
        #print("STRF = ", self.strf)
        a = self.document.paragraphs
        j = 0
        for i in self.strf:            
            print(i)
    ...
    def __parseJSON__(self):
        ...
        json_file = None
        try:
            json_file = open(self.path_to_JSON, "r")
        except FileNotFoundError:
            print("Указанный в настройках файл не найден")
            print("Путь до файла: ", self.path_to_JSON)
            return None
        ...
        for paragraph in self.document.paragraphs:
            ...
            if flg == 1:
                for r in paragraph.runs:
                    if (r.font.size.pt != self.settings["Font-size__Content-text_pt"]):
                        a = 1
                    if (r.font.name != None) and (r.font.name != self.settings["Font"]):
                        b = 1

                if (paragraph.paragraph_format.line_spacing != self.settings["Line-interval"]):
...
if __name__ == '__main__':
    ...
    for i in glob.glob('./test_document/*.docx'):
        check_documents.append(CheckDocument(i))

    for i in check_documents:
        print(i.way)
        print(i.studentInfo)
        print(i.errorsTitle)
        print(i.biblio)
